...
**Location**: `its_hub/integration/iaas.py:1`, `its_hub/core/reward_models`

**What Works**:
- OpenAI-compatible FastAPI server with `/v1/chat/completions` endpoint
- Support for self-consistency algorithm
- Budget-based inference-time scaling via request body parameter
- Async generation with `OpenAICompatibleLanguageModel`
...
- Extract orchestration logic from `iaas.py` into reusable `ITSGateway` class
- Define configuration schema at Request-level for models

**Step 2: ext_proc Service**
- Implement gRPC external processing service
- Integrate refactored `ITSGateway` with request/response handling
...
flowchart TD
      ...
      subgraph ITSHub["its_hub library"]
          ITSHubCore["ITS gateway<br/>(validation, algorithm, LM client)"]
      end
...
3. ext_proc forwards the request to a dedicated ITS Gateway instance.
4. The gateway reads ITS headers, runs the scaling algorithm using the refactored `its_hub` library, issues multiple LLM calls (via Envoy or direct)
5. Once the gateway selects the final response, it returns an OpenAI-compatible payload back to Envoy, which relays it to the client.
...
- **ITS Gateway Service**: stateless gRPC service that hosts the refactored ITS algorithms, manages per-request execution (budget enforcement, tool routing, failures), and coordinates reward scoring (TBD) through asynchronous workers.
...
1. Extract orchestration logic from `its_hub/integration/iaas.py` into a new module, e.g., `its_hub.core.gateway`.
   - Provide an `ITSGateway` class with methods `configure_model`, `run_chat_completion`, and `shutdown`.
...
2. Implement the service in Python (gRPC) or Go (for higher performance). Recommended layout:
   ...
   its_ext_proc/
     ...
     gateway_client.py # Thin wrapper around ITSGateway
     envoy/
...
3. Request handling flow inside `processor.py`:
   ...
   - Read/parse JSON body from `HttpBody` message. Forward to `ITSGateway.run_chat_completion()`.
   - Pass along client headers needed for downstream LLM auth (`Authorization`, `OpenAI-Organization`, etc.). Encode them in the gateway request context.
   - Short-circuit if ITS headers are absent: return `CONTINUE` so Envoy forwards request upstream without modification.
   - On ITS execution:
     1. Call gateway with budget/algorithm.
     2. Gateway issues N LLM calls. Calls should use Envoy cluster by making HTTP requests to `http://envoy-cluster-name/v1/chat/completions` (loop back through Envoy) or by leveraging direct provider clients with built-in retry policies.
     3. Gateway invokes evaluation logic; enforce timeouts and fallbacks.
...
4. Add resilience:
   - Wrap gateway calls with deadlines shorter than Envoy timeouts and issues upstream LLM calls concurrently.
...
### 4. Configure Envoy
1. Define clusters:
   - `its_ext_proc` (gRPC) → gateway service.
...
ITS gateway requires configurations, there are two ways we could approach this:
...
- Integration approach: implement a LiteLLM custom provider or pre-call hook that invokes the refactored `ITSGateway`. Downstream calls reuse LiteLLM’s provider clients, and reward services stay external.
...
- Integration approach: use Helicone transformers/webhooks to route ITS-specific requests to a gateway service that returns responses to Helicone for delivery.
- Pros: strong observability out of the box. Cons: requires external gateway for ITS loops; less control over inline request modification.

### Envoy/Kong Front with LiteLLM Core
- Run a managed ingress proxy (Envoy Gateway or Kong) for edge policies while delegating ITS gateway to LiteLLM behind it.
