...
class ParticleFilteringScene(Scene):
    def construct(self):
        ...
        particles = VGroup()
        for y in particle_y:
            p = Dot(radius=0.12, fill_color=ACCENT_BLUE, fill_opacity=0.8)
        ...
        for step in range(3):
            ...
            for i, p in enumerate(current_particles):
                new_p = Dot(radius=0.12, fill_color=ACCENT_BLUE, fill_opacity=0.8)
            ...
            self.play(*weight_anims, run_time=0.4)
            ...
            if resample_from[step] is not None:
                ...
                for i in range(n_particles):
                    src = sources[i]
                    if src != i:
                        copy_p = Dot(radius=0.12, fill_color=ACCENT_BLUE, fill_opacity=0.8)
                ...
                if dup_anims:
                    self.play(*dup_anims, run_time=0.4)
        ...
        self.play(ShowCreation(highlight), FadeIn(winner), run_time=0.5)
        self.wait(1.5)
