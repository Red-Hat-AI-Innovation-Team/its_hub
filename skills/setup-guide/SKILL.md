...
1. Explain what its_hub does: "its_hub is a library for inference-time scaling — it generates multiple LLM responses and selects the best one using voting or scoring algorithms."
...
3. If yes: install using the detected installer (`uv pip install "its_hub[lm]"` or `pip install "its_hub[lm]"`)
...
- Mention `/its-scale-batch` for batch processing.
