...
.codex-plugin/                # Codex CLI discovery manifest + install guide
  plugin.json
  INSTALL.md
skills/                       # Skills (auto-triggered + explicitly invokable)
...
scripts/                      # Bash helpers called by skills
  _env.sh                     # Shared venv resolution
  its_detect.sh               # Environment detection (library, config)
  its_scale.sh                # Execute scaling requests
```

Only `plugin.json` goes inside `.claude-plugin/` or `.codex-plugin/`. All other directories (`skills/`, `scripts/`) live at the repo root.

### Plugin Config

User config is stored at `.its-hub/config.json` (auto-generated by the `setup-guide` skill, gitignored).
