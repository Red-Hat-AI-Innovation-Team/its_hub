...
def main():
    ...
    for idx, dataset_row in enumerate(tqdm(problem_dataset, desc="Problems")):
        ...
        generations = [extract_content_from_lm_response(r) for r in result.responses]
        scores = result.scores
        token_counts = [len(tokenizer.encode(sol)) for sol in generations]
        ...
        for budget in budgets:
            ...
            solution = generations[best_idx]

            # Cumulative tokens for first N solutions
    ...
    for budget in budgets:
        ...
        if results:
            ...
            if correct_values:
                ...
                print(f"Budget {budget:3d}: {accuracy:.4f} ({int(n_correct):2d}/{int(n_total):2d}) | avg tokens: {avg_tokens:.0f}")
